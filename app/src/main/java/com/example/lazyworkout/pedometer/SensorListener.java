...
import com.example.lazyworkout.pedometer.util.API23Wrapper;
import com.example.lazyworkout.pedometer.util.Logger;
import com.example.lazyworkout.pedometer.util.Util;

import java.util.Date;

//TODO: change event.values[0] to event.values[0]
//TODO: change TYPE_STEP_COUNTER to type_step_counter
public class SensorListener extends Service implements SensorEventListener {

    public final static int NOTIFICATION_ID = 1;
    private final static long MICROSECONDS_IN_ONE_MINUTE = 60000000;
    private final static long SAVE_OFFSET_TIME = 5000; //TODO: AlarmManager.INTERVAL_HOUR
    private final static int SAVE_OFFSET_STEPS = 20;

    private static int steps;
    private static int lastSaveSteps;
    private static long lastSaveTime;

    // SHUT DOWN - IMPLEMENT LATER
...
//        int event.values[0] = (int) Math.sqrt(X * X + Y * Y + Z * Z);
...
//    }
    ...
    public int onStartCommand(final Intent intent, int flags, int startId) {
        reRegisterSensor();
//        registerBroadcastReceiver();  // SHUT DOWN - IMPLEMENT LATER
...
//
//    private static int steps;
//    private static float event.values[0];
//
//    @Override
...
//        double Y = event.values[1];
//        double Z = event.values[2];
//        event.values[0] = (float) Math.sqrt(X * X + Y * Y + Z * Z);
//        Log.d("stepCount", String.valueOf(event.values[0]));
//        getSharedPreferences("pedometer", Context.MODE_PRIVATE).edit()
//                .putFloat("event.values[0]", event.values[0]).apply();
//
////        steps = (int) event.values[0];
////        getSharedPreferences("pedometer", Context.MODE_PRIVATE).edit()
////                .putFloat("steps", steps).commit();
//    }
//
//    @Override
