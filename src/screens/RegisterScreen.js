...
import {
  View, TouchableOpacity, Pressable, StyleSheet, Text, Keyboard, SafeAreaView,
...
export default RegisterScreen = ({ navigation }) => {
  ...
  const handleRegister = () => {
    ...
    Authentication.createAccount(
      ...
      (user) => Alert.alert(
        "Verification",
        "Last step: Verify your email before logging in with your new account!",
        [
          {
            text: "OK", onPress: () =>
              navigation.dispatch(CommonActions.reset({
                ...
                routes: [{
                  name: "Login",
                  params: { name: user.displayName, email: user.email }
                }]
              }))
          }
        ]
      ...
      (error) => {
        ...
        let errorMsg = Authentication.registerError(error);
        setError(errorMsg);
      }
    );
